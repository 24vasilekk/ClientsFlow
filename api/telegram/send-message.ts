...
async function fetchTelegramWithRetry(url: string, init: RequestInit, attempts = 3, timeoutMs = 10000): Promise<Response> {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      ...
      console.warn("[telegram/send-message] retry_status", { status: response.status, attempt });
    } catch (error: any) {
      lastError = error;
      const logTag = error?.name === "AbortError" ? "retry_timeout" : "retry_network_error";
      console.warn(`[telegram/send-message] ${logTag}`, { attempt, message: error?.message || "unknown_error" });
  ...
  throw lastError || new Error("telegram_send_message_failed");
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!response.ok || data?.ok !== true) {
      console.error("[telegram/send-message] api_error", {
    ...
    console.error("[telegram/send-message] handler_error", { message: error?.message || "unknown_error", chatId: String(chatId) });
