...
export default async function handler(req: any, res: any) {
  ...
  const traceId = String(req.headers?.["x-trace-id"] || req.body?.traceId || `trace_business_audit_${Date.now().toString(36)}`);
  try {
    await requireRequestContext(req, "api/openrouter/business-audit");
  } catch (error: any) {
