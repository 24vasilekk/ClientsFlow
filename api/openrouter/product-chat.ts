...
export default async function handler(req: any, res: any) {
  ...
  const traceId = String(req.headers?.["x-trace-id"] || req.body?.traceId || `trace_product_chat_${Date.now().toString(36)}`);
  try {
    await requireRequestContext(req, "api/openrouter/product-chat");
  } catch (error: any) {
